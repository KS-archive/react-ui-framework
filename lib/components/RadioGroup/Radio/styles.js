...
})(["cursor:pointer;display:flex;color:var(--grey2);&:hover{> div::before{opacity:0.25;}}"]);
...
})(["top:1px;margin-left:var(--space-sm);font-weight:var(--regular);font-size:var(--font-md);transition:all 0.3s linear;"]);
...
var Check = _styledComponents.default.div.withConfig({
  ...
  return error && typeof error === 'string' ? '1px solid var(--error)' : '1px solid var(--grey2)';
}, function (_ref2) {
  var error = _ref2.error;
  return error && typeof error === 'string' ? 'var(--error)' : 'var(--primary2)';
});
